...
const uint8_t PAN211_TestAddr[5] = {0xC1,0x31,0x21,0x46,0x33};
 ...
 * @return None
 */
void PAN211_WriteReg(uint8_t Reg, uint8_t Value)
{
    SPI_CS_LOW;
    SPI_WriteByte(((Reg << 1) | 0x01)); /* BIT7~1 register address, BIT0=1 indicates write operation */
    SPI_WriteByte(Value);
    SPI_CS_HIGH;
}
 ...
 */
uint8_t PAN211_ReadReg(uint8_t Reg)
{
    uint8_t Value;
    SPI_CS_LOW;
    SPI_WriteByte((Reg << 1)&0xFE);   /* BIT7~1 register address, BIT0=0 indicates read operation */
    Value = SPI_ReadByte();
    SPI_CS_HIGH;
    return Value;
}
 ...
 */
void PAN211_WriteRegs(uint8_t Reg, const uint8_t *Buffer, uint8_t Len)
{
    SPI_CS_LOW;
    SPI_WriteByte(((Reg << 1) | 0x01));
    for (int i = 0; i < Len; i++)
    {
 ...
 */
void PAN211_ReadRegs(uint8_t Reg, uint8_t *Buffer, uint8_t Len)
{
    SPI_CS_LOW;                        /* 拉低CS引脚，开始SPI传输 */
    SPI_WriteByte((Reg << 1) & 0xFE); /* BIT7~1寄存器地址, BIT0=0表示读操作 */
    for (int i = 0; i < Len; i++)
    {
...
#endif
    PAN211_WriteRegs(PAN211_P0_PIPE1_RXADDR0_CFG, PAN211_TestAddr, 5);
    // Read back and check
    PAN211_ReadRegs(PAN211_P0_PIPE1_RXADDR0_CFG, tmp, 5);
    for (i = 0; i < 5; i++) 
    {
        printf("%02X", *(tmp + i));
        if (*(tmp + i) != *(PAN211_TestAddr + i)) return 0;
    }
    return 1;
}
...
void PAN211_SetEnhancedMode(uint8_t enabled, PAN211_TxAuto_Ard_t ard, PAN211_TxAuto_Arc_t arc)
{
    ...
    else
    {
        ...
        PAN211_WriteReg(PAN211_P0_TXAUTO_CFG, (ard << 4) | arc); // 0000 0011,  0x00:250µs+noack, 0x03:250µs+3retry
...
{
    PAN211_WriteReg(PAN211_P0_RX_PLLEN_CFG, len);
}
...
{
    PAN211_WriteReg(PAN211_P0_TX_PLLEN_CFG, len);
}
...
{
    PAN211_WriteReg(PAN211_P0_TRXTWTL_CFG, us & 0xFF);
    PAN211_WriteReg(PAN211_P0_TRXTWTH_CFG, (us >> 8) & 0xFF);
}
 ...
 * @param pipes 0x01:p0, 0x03:p0,p1, ..., 0x3F:p0~p5
 */
void PAN211_EnableRxPipes(uint8_t pipes)
{
    PAN211_WriteReg(PAN211_P0_RXPIPE_CFG, pipes); // [5]PIPE5_EN [4]PIPE4_EN [3]PIPE3_EN [2]PIPE2_EN [1]PIPE1_EN [0]PIPE0_EN
}
 ...
 * @param Len Address length
 */
void PAN211_SetTxAddr(const uint8_t *Addr, uint8_t Len)
{
